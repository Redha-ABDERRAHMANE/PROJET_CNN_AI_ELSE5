			...
			for (short x = 0; x < IMG_HEIGHT - CONV1_DIM + 1; x += CONV1_STRIDE) {
				...
				for (short ky = 0; ky < CONV1_DIM; ky += CONV1_STRIDE) {
					for (short kx = 0; kx < CONV1_DIM; kx += CONV1_STRIDE) {
						res += fixed_mul(input[0][y + ky][x + kx] , kernel[filter_index][0][ky][kx]); // 0 because the depth is 1 so index 0 
					}
				}
...
void Conv2_12x12x20_5x5x40_1_0(int32_t input[POOL1_NBOUTPUT][POOL1_HEIGHT][POOL1_WIDTH], 	            // IN
	int32_t kernel[CONV2_NBOUTPUT][POOL1_NBOUTPUT][CONV2_DIM][CONV2_DIM], 	// IN
				...
				for (short z = 0; z < POOL1_NBOUTPUT; z++) {
					for (short ky = 0; ky < CONV2_DIM; ky += CONV2_STRIDE) {
						for (short kx = 0; kx < CONV2_DIM; kx += CONV2_STRIDE) {
							res += fixed_mul(input[z][y + ky][x + kx],kernel[filter_index][z][ky][kx]); // 0 because the depth is 1 so index 0 
						}
					}
				}
...
void Fc1_40_400(int32_t 	input[POOL2_NBOUTPUT][POOL2_HEIGHT][POOL2_WIDTH], 			        // IN
	int32_t 	kernel[FC1_NBOUTPUT][POOL2_NBOUTPUT][POOL2_HEIGHT][POOL2_WIDTH],	// IN
		...
		for (short z = 0; z < POOL2_NBOUTPUT; z++)
			for (short y = 0; y < POOL2_HEIGHT; y++)
				for (short x = 0; x < POOL2_WIDTH; x++)
					sum += fixed_mul(input[z][y][x] , kernel[k][z][y][x]);
